...
### Phase 1: 约束检查 + 选择下一个实验

**⚠️ 首先读取硬性约束:**
读取 `competition/COMPETITION_ANALYSIS.md` 中的 "硬性约束" 部分。
所有实验方案必须在约束范围内，包括但不限于:
- 模型参数量限制 (例如 ≤20B → 不能用 70B/72B 模型)
- 推理时间/硬件限制
- 禁止使用的资源

**如果某个实验方案违反硬性约束，直接跳过，不要实现。**

从 `EXPERIMENT_PATH.md` 中选择下一个要执行的实验:

**选择策略:**
1. **先过滤**: 排除违反硬性约束的实验
2. 按优先级: MUST-RUN > SHOULD-TRY > NICE-TO-HAVE
3. 按依赖: 先完成前置实验
4. 按路径表现: 优先选择历史表现好的路径
5. 跳过已完成的实验
